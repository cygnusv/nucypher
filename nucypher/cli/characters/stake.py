...
class StakeHolderCharacterOptions:
...
group_character_options = group_options(
    StakeHolderCharacterOptions,
    config_options=group_config_options,
    staking_address=option_staking_address,
    )


class PreallocationStakeHolderCharacterOptions:

    __option_name__ = 'prealloc_character_options'

    def __init__(self, character_options, hw_wallet, beneficiary_address, allocation_filepath):
    ...
    def get_blockchain(self):
        return self.character_options.get_blockchain()
...
group_prealloc_character_options = group_options(
    PreallocationStakeHolderCharacterOptions,
    character_options=group_character_options,
...
@group_general_config
def list(general_config, character_options, config_file, all):
    """
...
def set_worker(general_config, prealloc_character_options, config_file, force, worker_address):
    """
    ...
    client_account, staking_address = handle_client_account_for_staking(
        ...
        staking_address=prealloc_character_options.character_options.staking_address,
    ...
    password = prealloc_character_options.get_password(blockchain, client_account)
...
@option_force
@group_general_config
def detach_worker(general_config, prealloc_character_options, config_file, force):
    """
...
@option_lock_periods
@group_general_config
def create(general_config, prealloc_character_options, config_file, force, value, lock_periods):
    """
...
@option_force
@group_general_config
def restake(general_config, prealloc_character_options, config_file, enable, lock_until, force):
    """
...
@group_general_config
def divide(general_config, prealloc_character_options, config_file, force, value, lock_periods, index):
    """
    ...
    if prealloc_character_options.character_options.staking_address and index is not None:  # 0 is valid.
        STAKEHOLDER.stakes = StakeList(
            registry=STAKEHOLDER.registry,
            checksum_address=prealloc_character_options.character_options.staking_address)
...
@option_force
@group_general_config
def collect_reward(general_config, prealloc_character_options, config_file,
...
@option_force
@group_general_config
def preallocation(general_config, prealloc_character_options, config_file, action, force):
    """
